...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device, logger, config, experiment_log_dir, training_mode):
    ...
    for epoch in tqdm(range(1, config.num_epoch + 1), position=0, leave=True):
        ...
        if 'self_supervised' not in training_mode:  # use scheduler in all other modes.
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode):
    ...
    for batch_idx, (data, labels, aug1, aug2) in tqdm(enumerate(train_loader), position=0, leave=True):
        # send to device
        ...
        if 'self_supervised' in training_mode:
            predictions1, features1 = model(aug1.cuda())
            predictions2, features2 = model(aug2.cuda())
...
def model_evaluate(model, temporal_contr_model, test_dl, device, training_mode, config=None):
    model.eval()
    ...
    with torch.no_grad():
        for data, labels, aug1, aug2 in tqdm(test_dl, position=0, leave=True):
            #data, labels = data.float().to(device), labels.long().to(device)
            data, labels = data.float(), labels.long()

            if 'self_supervised' in training_mode:
                predictions1, features1 = model(aug1.float().cuda())
                predictions2, features2 = model(aug2.float().cuda())
