...
from dataloader.dataloader import data_generator, data_generator_larfield
from trainer.trainer import Trainer, model_evaluate
from models.TC import TC
from utils import _calc_metrics, copy_Files
from models.model import base_Model
#from config_files.LarField_Configs import Config
# Args selections
...
configs = Configs()
datasets_names = ["HAR", "Epilepsy", "sleepEDF", "pFD"]
"""
h = [32, 50, 64, 100, 128, 200, 256]
jitter_scale_ratio = [1.1, 0.001, 1.5, 2]
jitter_ratio = [0.8, 0.001, 2, 0.1]
max_seg = [8, 5, 12, 5]
"""
h = [100]
timesteps = [10]
jitter_scale_ratio = [0.01]
jitter_ratio = [0.001]
lrs = [3e-4]
max_seg = [5]
for lr in tqdm(lrs):

    for i in tqdm(range(len(jitter_scale_ratio))):
        ...
        run_description = f"jitter_scale_ratio_{jitter_scale_ratio[i]}"

        configs.augmentation.jitter_scale_ratio = jitter_scale_ratio[i]
        configs.augmentation.jitter_ratio = jitter_ratio[0]
        configs.augmentation.max_seg = max_seg[0]
        configs.lr = lr
        configs.Context_Cont.hidden_dim = h[0]
        configs.Context_Cont.timesteps = timesteps[0]
        ...
        # Load datasets
        data_path = f"../../LarFieldData/i_04_mult/dataset_samples/"
        #train_dl, valid_dl, test_dl = data_generator(data_path, configs, training_mode)
        train_dl, valid_dl, test_dl = data_generator_larfield(data_path, configs, training_mode)
